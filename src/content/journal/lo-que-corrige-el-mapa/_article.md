...
So: the channel is real but unreliable. And here's where it gets interesting, because the same problem turns up, mirrored, when you look at it from the other side.
...
Bateson's phrase for this was, depending on which essay you read, *the difference that makes a difference*, or *information* in the strict cybernetic sense. He meant: a signal is informative only if it could have been otherwise, and the system is structured to be moved by it. A closed loop has no information. An open loop, embedded in a world that pushes back, has as much as it can absorb.
