 ...
 * 
 * This utility helps identify issues with Jekyll sites, particularly
 * those using the Minimal Mistakes theme. It checks for configuration
 * problems, missing dependencies, and structural issues.
 * 
 * Usage: 
...
class JekyllDiagnostics {
  ...
  checkStyles() {
    ...
    let varsMissing = false;
    
    customVars.forEach(varName => {
      const value = cssVars.getPropertyValue(varName).trim();
      if (!value) {
        this.addIssue('styles', 'warn', `CSS variable ${varName} is not defined`, 
          'CSS variables are required for the theme to function properly.');
        varsMissing = true;
      }
    });
  ...
  checkScripts() {
    ...
    if (!mainScript) {
      this.addIssue('scripts', 'warn', 'Main script not detected', 
        'The main.js file is not detected. Some functionality may be limited.');
    } else {
      console.log('✓ Main script detected');
    }
    
    // Check for modules import support
    const moduleSupport = 'noModule' in document.createElement('script');
    if (!moduleSupport) {
      this.addIssue('scripts', 'warn', 'ES modules may not be supported', 
        'Your browser may not support ES modules. Some JavaScript functionality might not work.');
    } else {
      console.log('✓ ES modules are supported');
    }
    
    // Check for JS errors
    const errorCount = window.jsErrors || 0;
    if (errorCount > 0) {
